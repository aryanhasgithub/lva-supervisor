...
AUDIO_AGENT_TIMEOUT = aiohttp.ClientTimeout(total=10)

routes = web.RouteTableDef()


async def _query_audio_agent() -> dict[str, list[str]]:
    ...
    async with aiohttp.ClientSession(
        connector=connector, timeout=AUDIO_AGENT_TIMEOUT
    ) as session:
        async with session.get(AUDIO_AGENT_URL) as resp:
            if resp.status != 200:
                raise RuntimeError(f"Audio agent returned status {resp.status}")
...
async def get_audio_devices(
    ...
    try:
        ...
        _LOGGER.warning("Audio agent timed out after %ss", AUDIO_AGENT_TIMEOUT.total)
        return web.json_response(
            {"error": "lva-audio agent did not respond in time"},
            status=503,
        )
